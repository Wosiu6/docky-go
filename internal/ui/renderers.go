...
func renderPostgresFancy(container fetcher.ContainerInfo, width, height int) string {
	...
	content.WriteString(titleStyle.Background(colorBorder).Width(width-4).Render(fmt.Sprintf("%s %s", icon, name)) + "\n")
	if dbName != "" {
		content.WriteString(lipgloss.NewStyle().Foreground(colorBorder).Bold(true).Render("DB: "+dbName) + "\n")
	}
	...
	styledBox := containerStyle.BorderForeground(colorBorder).BorderStyle(lipgloss.DoubleBorder()).Width(width).Height(height).Render(content.String())
	return styledBox
}
...
func renderMinecraftFancy(container fetcher.ContainerInfo, width, height int) string {
	...
	content.WriteString(titleStyle.Background(colorBorder).Width(width-4).Render(fmt.Sprintf("%s %s", icon, name)) + "\n")
	if version != "" {
		content.WriteString(lipgloss.NewStyle().Foreground(colorBorder).Bold(true).Render("Version: "+version) + "\n")
	}
	if players != "" {
		content.WriteString(lipgloss.NewStyle().Foreground(colorBorder).Bold(true).Render("Players: "+players) + "\n")
	}
	colorHex, statusIcon, statusText := StatusInfo(container.Status)
	content.WriteString(statusStyle.Foreground(lipgloss.Color(colorHex)).Render(fmt.Sprintf("%s %s", statusIcon, statusText)) + "\n")
	content.WriteString(statsStyle.Render(fmt.Sprintf("CPU: %.1f%%  MEM: %dMB", container.CPUPercent, container.Mem)) + "\n")
	...
	styledBox := containerStyle.BorderForeground(colorBorder).BorderStyle(pixelBorder).Width(width).Height(height).Render(content.String())
	return styledBox
}
...
func renderGenericFancy(container fetcher.ContainerInfo, width, height int) string {
	...
	var content strings.Builder
	content.WriteString(titleStyle.Background(colorBorder).Width(width-4).Render(fmt.Sprintf("%s %s", icon, name)) + "\n")
	typeBadge := lipgloss.NewStyle().Foreground(colorBorder).Bold(true).Render(fmt.Sprintf("\u25cf %s", typeLabel))
	...
	styledBox := containerStyle.BorderForeground(colorBorder).Width(width).Height(height).Render(content.String())
	return styledBox
}
...
func renderTraefikFancy(c fetcher.ContainerInfo, w, h int) string {
	...
	b.WriteString(titleStyle.Background(colorBorder).Width(w-4).Render(fmt.Sprintf("%s %s", icon, name)) + "\n")
	...
	return containerStyle.BorderForeground(colorBorder).BorderStyle(lipgloss.DoubleBorder()).Width(w).Height(h).Render(b.String())
}
...
func renderRedisFancy(c fetcher.ContainerInfo, w, h int) string {
	...
	return containerStyle.BorderForeground(colorBorder).BorderStyle(lipgloss.ThickBorder()).Width(w).Height(h).Render(b.String())
}
...
func renderMinioFancy(c fetcher.ContainerInfo, w, h int) string {
	...
	return containerStyle.BorderForeground(colorBorder).BorderStyle(border).Width(w).Height(h).Render(b.String())
}
...
func renderGrafanaFancy(c fetcher.ContainerInfo, w, h int) string {
	...
	return containerStyle.BorderForeground(colorBorder).BorderStyle(lipgloss.RoundedBorder()).Width(w).Height(h).Render(b.String())
}
